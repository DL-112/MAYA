...
"""Burmese Language Related Functions"""

# Syllable-breaking code from sylbreak.py
...
def syl_break_and_utf8(text):
    input_string = text
    
    # Perform syllable breaking
    broken_text = break_syllables(input_string)
    
    # Encode syllables to UTF-8 bytes and get byte codes as list
    syllables = broken_text.split()
    unicode_byte_values = []
    
    for syllable in syllables:
        # Convert syllable to bytes (UTF-8)
        utf8_bytes = list(syllable.encode('utf-8'))
        unicode_byte_values.extend(utf8_bytes)  # Add the byte values to the list
